...
export function StatusStepper({ kind, value, onStep }: StatusStepperProps) {
  return (
    <div className="flex h-7.5 items-center rounded-full border border-white/8 bg-elevated">
      <button type="button" aria-label="Previous status" onClick={() => onStep(-1)} className={arrow}>
      ...
      <button type="button" aria-label="Next status" onClick={() => onStep(1)} className={arrow}>
