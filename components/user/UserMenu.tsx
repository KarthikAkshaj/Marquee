...
function MenuItem({ children, danger = false, disabled, onSelect }: { children: string; danger?: boolean; disabled?: boolean; onSelect: () => void }) {
  return (
    <DropdownMenu.Item
      disabled={disabled}
      onSelect={onSelect}
      className={cn(
        ...
        danger ? "text-dropped-muted data-highlighted:bg-dropped-muted/12 data-highlighted:text-dropped" : "text-text data-highlighted:bg-white/5",
      )}
    >
...
function LinkItem({ href, children }: { href: string; children: string }) {
  return (
    <DropdownMenu.Item asChild>
      <Link
        href={href}
        className="flex cursor-pointer items-center gap-2.25 rounded-nav px-2.5 py-2.25 text-13 text-text outline-none select-none data-highlighted:bg-white/5"
      >
